...
const SwipeToDelete = ({
  ...
  const onMove = useCallback(
    function (event: TouchEvent | MouseEvent) {
      ...
      lastPosition.current = cursorPosition(event);
    },
    [rtl, touching]
  );
  ...
  const onMouseUp = useCallback(
    function () {
      if (
        lastPosition.current == 0 || // Didn't move at all
        (lastPosition.current < startTouchPosition.current + 10 &&
          lastPosition.current > startTouchPosition.current - 10) // Moved only a little
      ) {
        onClick?.();
      }
